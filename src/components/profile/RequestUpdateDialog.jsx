...
const RequestUpdateDialog = ({ open, onClose, onSuccess }) => {
  ...
  const handleSubmitRequest = async () => {
    ...
    try {
      ...
      // Check if the error is because user already has permission
      if (error.response?.status === 400 && 
          error.response?.data?.message === "You already have permission to update your profile") {
        // This is actually not an error - the user can edit their profile
        if (onSuccess) {
          onSuccess({
            message: "You already have permission to update your profile",
  ...
  return (
    ...
    >
      <DialogTitle className={classes.dialogTitle}>
        Request Profile Update Permission
      </DialogTitle>
      <DialogContent className={classes.dialogContent}>
        ...
        <TextField
          ...
          required
          error={!!error}
          helperText={error ? null : "Minimum 10 characters required"}
          disabled={submitting}
        />
      </DialogContent>
      <DialogActions>
